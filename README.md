...
- **`/help`** — справка по /draft
- **`/remove`** — удалить участника(ов) из драфта (только создатель, до 5 пользователей за раз)
- **`/rename`** — переименовать драфт (только создатель)
...
- (опционально) `DRAFT_RETENTION_DAYS` — авто-удаление драфтов старше N дней (по умолчанию 7)
